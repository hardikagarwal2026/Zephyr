...
REDIRECT_URI = "https://zephyrspacebyhardikagarwal.streamlit.app/"
...
if 'token' not in st.session_state:
    ...
    result = oauth2.authorize_button("Login with Google", REDIRECT_URI, scope_str)
...
def search_books(query, api_key, access_token=None):
...
if query:
    ...
    books = search_books(query, BOOKS_API_KEY, access_token)
    if books:
        for book in books:
...
# Generate a personalized reading list based on preferences
